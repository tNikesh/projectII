...
<body class="bg-white w-screen h-screen p-0 m-0 overflow-x-hidden">
  ...
  <main class="w-[83%] absolute right-0 top-0 p-5">
   <section>
    <h1 class="w-full text-left font-medium text-lg my-7 ">Sub Category</h1>
    <div class="w-full bg-gray-50 drop-shadow-lg py-10 px-7">
      <x-forms.button id="sub-category" content="new sub category" class="w-56 mb-14 bg-white text-gray-900 ring-1 ring-black hover:bg-gray-800 hover:text-white"/>
   ...
   <section id="category-modal" class="z-50 absolute top-0 left-1/2 -translate-x-1/2 -translate-y-full w-full max-w-[550px] py-8 px-5 bg-white border-2 border-black flex flex-col justify-center items-center gap-y-10 drop-shadow-xl transform transition-all duration-300 ease-in-out"  >
    <h1 class="w-full text-left uppercase text-lg font-semibold ">sub category</h1>

    <form class="flex flex-col justify-center items-center w-full gap-y-7 px-7"  action="{{ route('post.sub.catgeory') }}" method="POST">
      @csrf
      <div class="flex flex-col justify-center items-start  w-full ">
        <x-forms.label for="title" content="sub title" class=""/>
        <x-forms.input name="title" placeholder="Enter sub title" class="w-full text-gray-700 bg-white" autofocus/>
      </div>
      <div class="flex flex-col justify-center items-start w-full">
        <x-forms.label for="desc" content="sub Description"/>
        <x-forms.input name="desc" placeholder="Enter sub description" class="w-full text-gray-700 bg-white"/>
      </div>
     <div class="flex justify-center items-center gap-x-2 w-full">
      ...
      <x-forms.button type="submit" content="create sub category" class="w-full py-3 ring-1 ring-gray-800 hover:text-gray-800 hover:bg-white"/>
     </div>
    </form>
   </section>
  </main>
  @stack('scripts')
  <script>
    const openModal=document.getElementById('sub-category');
